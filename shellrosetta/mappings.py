...
LINUX_TO_PS = {
    ...
    "ls -lh": (
        ...
        "Human-readable file sizes not natively available.",
    ),
    ...
    "chmod": (
        "icacls",
        "Use 'icacls file /grant user:permissions'. Not as granular/flexible as Linux.",
    ),
    ...
    "tar -czvf": (
        "Compress-Archive",
        "Outputs .zip, not .tar.gz. For gzip, use third-party modules or 7-Zip.",
    ),
    "tar -czvf backup.tar.gz myfolder/": (
        "Compress-Archive -Path myfolder/ -DestinationPath backup.zip",
        "Outputs .zip, not .tar.gz. For gzip, use third-party modules or 7-Zip.",
    ),
    ...
    "top": (
        ...
        "Use Get-Process | Out-GridView for a dynamic view.",
    ),
    ...
    "du -sh": (
        "(Get-ChildItem dir -Recurse | Measure-Object -Property Length -Sum).Sum",
        None,
    ),
    ...
    "lspci": (
        "Get-CimInstance Win32_PnPEntity | Where-Object { $_.PNPClass -eq 'PCI' }",
        None,
    ),
    "lsusb": (
        "Get-PnpDevice -PresentOnly | Where-Object { $_.Service -eq 'USBSTOR' }",
        None,
    ),
...
PS_TO_LINUX = {
    ...
    "Compress-Archive": (
        "tar -czvf",
        "tar creates .tar.gz; Compress-Archive creates .zip",
    ),
